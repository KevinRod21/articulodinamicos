...
x7 = cereales$`Produccion de cereal (EEUU)`/10000000
n = length(y)
...
m1.model <-function() {
  ...
  w1[1]~dgamma(0.1,0.1)
  w2[1]~dgamma(0.1,0.1)
  alp[1]~dgamma(0.1,0.1)
  for(i in 2:n)
  {
    y[i]~dgamma(alp[i],alp[i]/mu[i])
    log(mu[i])<- beta0[i]+beta1[i]*x1[i]
    beta0[i]~dnorm(beta0[i-1],w1[i])
    beta1[i]~dnorm(beta1[i-1],w2[i])
    alp[i]~dgamma(0.1,0.1)
    w1[i]~dgamma(0.1,0.1)
    w2[i]~dgamma(0.1,0.1)
  }
}
m1.data <- list("y","n","x1")
m1.param <- c("mu","beta0","beta1")
set.seed(123)
...
plot(media,type="l",ylim=c(a1,a2),col="aquamarine3",lwd=2, 
     main = "Intervalos de credibilidad al 95% para M1")
...
media=Resumen[(2*n+2):(3*n),1]
li=Resumen[(2*n+2):(3*n),3]
ls=Resumen[(2*n+2):(3*n),7]
a1=min(li,ls)-1
a2=max(li,ls)+1
plot(media,type="l",ylim=c(a1,a2),col="aquamarine3",lwd=2,
     main = "Intervalos de credibilidad al 95% para B3 de M1")
lines(li,type="l",col="red",lty=2)
lines(ls,type="l",col="red",lty=2)
abline(h=0,lty=2,col="gray40") #No es significativo

#  Estimación de  $\beta_{3t}$
...
lines(y[c(2:47)],type="l",lwd=2,col="gray40")
legend("topright", legend = c("Estimado","Acción"),
